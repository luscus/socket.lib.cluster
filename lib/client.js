...
function updatePool (url, socket, status) {
  ...
  //console.log('updatePool', url, status);
  ...
  if (status && index < 0) {
    ...
    socket.connections[url].connected = true;
    ...
    socket.connections[url].connected = false;
    socket.connections[url].nextReconnect = Date.now();
    socket.connections[url].nextReconnectOffset = 2000;
  ...
  if (hasChanged) {
    socket.emit('pool_update', url, status);
  }
}
...
var clientTemplate = {
  ...
  updateStats: function (url, data) {
    ...
    //       console.log('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');
    //       console.log('queue length: ', this.getQueueLength(url));
